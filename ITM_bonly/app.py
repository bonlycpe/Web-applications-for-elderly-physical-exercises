...
modelLagRaises=pickle.load(open(r"model/lagRaises.pkl", 'rb'))
...
mpModel=[modelWaistFeetAndLegRaises,modelStompingAndBent,modelHandUp,modelHandUp]
countGoal = [5,10,15]
start_time = time.time()
        ...
        try:
            ...
            # A Course
                ...
                if(count==countGoal[0]):
                    setZero()
                    nowCourse=course[courseSelect][status]
                if(stage == 1 and predict_class=="wflr1" and round(predict_prob[np.argmax(predict_prob)],2) >= 0.55):
                    if(step==1):
                        stage = 2
                    elif(step==2):
                        stage = 3
                elif(stage == 2 and predict_class=="wflr2" and round(predict_prob[np.argmax(predict_prob)],2) >= 0.55):
                    step=2
                    stage = 1
                elif(stage == 3 and predict_class=="wflr3" and round(predict_prob[np.argmax(predict_prob)],2) >= 0.55):
                    count+=1
                    step = 1
                    stage = 1
            elif(nowCourse==course[0][2]):
